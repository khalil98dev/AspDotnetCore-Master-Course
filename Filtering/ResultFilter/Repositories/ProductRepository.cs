...
public class ProductRepository(ProductAppContext context) : IProductRepository
{
    ...
    public async Task<bool> AddProductAsync(Product product,CancellationToken ct)
    {
        if (product is null)
            return false;

        if (await ExistsByIdAsync(product.Id,ct) || await ExistsByNameAsync(product.Name,ct))
            return false;
    ...
    public async Task<bool> AddProductReviewAsync(ProductReview review,CancellationToken ct)
    {
    ...
    public async Task<bool> UpdateProductAsync(Product updatedProduct,CancellationToken ct)
    {
        ...
        return await context.SaveChangesAsync(ct) > 1; // Return true if at least one row was affected
    }
